...
function VitalsForm({ onSubmit, loading }) {
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); onSubmit({ ...form, entryDate: new Date(form.entryDate).toISOString() }); }}>
      <Field label="Date"><Input id="vital-date" type="date" value={form.entryDate} onChange={set('entryDate')} /></Field>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        <Field label="Tension artérielle"><Input id="vital-bp" value={form.bloodPressure} onChange={set('bloodPressure')} placeholder="Ex: 120/80" /></Field>
        <Field label="Fréquence cardiaque"><Input id="vital-hr" type="number" value={form.heartRate} onChange={set('heartRate')} placeholder="bpm" /></Field>
        <Field label="Température (°C)"><Input id="vital-temp" type="number" step="0.1" value={form.temperature} onChange={set('temperature')} placeholder="Ex: 37.2" /></Field>
...
export default function HealthNotebookSection({ patientId }) {
  ...
  return (
    <>
      <Card>
        <SectionHeader icon="heart" title="Carnet de santé" count={list.length} onAdd={isDoctor ? () => setModal(true) : null} addLabel="Saisir constantes" color="#059669" />
        ...
        {!loading && latest && (
          <div style={{ padding: '16px 24px', borderBottom: '1px solid #f3f4f6' }}>
            <div style={{ fontSize: '0.75rem', color: '#9ca3af', fontWeight: 600, marginBottom: 12, textTransform: 'uppercase', letterSpacing: '0.04em' }}>
              Dernières constantes · {fmtDate(latest.entryDate)}
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: 10 }}>
              <VitalWidget icon="droplet" label="Tension" value={latest.bloodPressure} unit="mmHg" color="#dc2626" />
              <VitalWidget icon="heart" label="Fréq. cardiaque" value={latest.heartRate} unit="bpm" color="#ef4444" />
              <VitalWidget icon="thermometer" label="Température" value={latest.temperature} unit="°C" color="#d97706" />
        ...
        {!loading && !error && list.length === 0 && (
          <EmptyState icon="heart" message="Aucune constante enregistrée" sub={isDoctor ? "Commencez à suivre les signes vitaux." : "Aucune donnée de santé disponible."} />
        )}
        ...
        {!loading && list.map((e, i) => (
          <DataRow key={e.id || i}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontWeight: 600, color: '#111827', fontSize: '0.88rem' }}>{fmtDate(e.entryDate)}</div>
